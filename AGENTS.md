...
1. **Scan Directory Structure & Stack:** Read `package.json`, `Package.swift`, `app/`, `src/`, config files, and DB schemas.
...
* **No Hardcoded Hex Values:** Always reference tokens from `context/ui-tokens.md`.
* **Boundary Respect:** Keep UI logic separate from API/server handlers, and avoid direct DB calls inside client UI components.
* **Skill Usage:** Use `architect` before complex tasks, `review` after major builds, `recover` on errors, `imprint` for UI patterns, and `remember` at session ends.
